  ...
  return {
    alert: lead
      ? {
          ...
          copy: `${possessive(lead.competitor)} latest ${lead.sourceLabel.toLowerCase()} signal is pushing a fresh modernization story. Publish one response asset quickly so the market sees Netezza as the performant warehouse engine for governed lakehouse analytics in the same decision window.`,
        }
      : {
  ...
  return {
    recommendation: {
      ...
      evidence: recommendationLead
        ? `Latest proof: ${possessive(recommendationLead.competitor)} ${recommendationLead.sourceLabel.toLowerCase()} signal is reinforcing ${extractTheme(recommendationLead)}. That keeps Netezza's best response anchored in lakehouse-ready performance, control, and workload-fit credibility.`
        : "Latest proof is unavailable, so the positioning guidance is staying anchored to the stable lakehouse performance-engine narrative.",
    },
...
function createCounterPostAction(signal) {
  return {
    ...
    summary: `Create a short executive-ready post that answers ${possessive(signal.competitor)} latest ${signal.sourceLabel.toLowerCase()} message with a clearer Netezza point of view.`,
    status: "Urgent",
    citations: [toSignalCitation(signal)],
    coverageType: signal.coverageType || "static",
    coverageLabel: signal.coverageLabel || "Static",
...
function createExecutiveBriefingAction(signals) {
  ...
  return {
    ...
    status: "",
    citations: signals.map(toSignalCitation),
...
function createAnalystBriefingAction(signals) {
  return {
    ...
    summary: "Package the latest competitive narratives into a concise analyst briefing memo so category framing includes the warehouse engine a lakehouse needs.",
    status: "",
    citations: signals.map(toSignalCitation),
    coverageType: signals.some((signal) => signal.coverageType === "live") ? "live" : "static",
    coverageLabel: signals.some((signal) => signal.coverageType === "live") ? "Live" : "Static",
