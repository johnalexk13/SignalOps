...
- `Launch SignalOps Dashboard.command`: one-click local launcher
...
- `Launch SignalOps Dashboard.command`

## Account workspace links

Use account links when different people should see different product libraries.

Example:

```text
http://localhost:3000/?account=john&products=ibm-netezza
```

The `account` value keeps saved workspace data separate in the browser. The `products` value controls which focus products appear in that person's Product Library. The default launcher now starts with Netezza only, and new products can be added later from Manage.
